...
// ── Email 1b — Order Confirmed (3D configurator orders) ─────────────────────
// Sent by /api/order-3d-paid when the design server's payment webhook fires.
// Unlike CONFIRMED, these orders carry the actual jersey renders (front/back
// snapshots the tool uploaded on save) and the per-player roster — showing
// them is the whole point: the store platform's own email can only show the
// generic product photo, which scares customers ("this is not what I made").

const ROSTER_LABELS = {
  en: { number: '#', name: 'Name', size: 'Size', qty: 'Qty' },
  es: { number: 'Dorsal', name: 'Nombre', size: 'Talla', qty: 'Cant.' },
  fr: { number: 'N°', name: 'Nom', size: 'Taille', qty: 'Qté' },
  it: { number: 'Numero', name: 'Nome', size: 'Taglia', qty: 'Qtà' },
};

function rosterTable(players, lang) {
  if (!Array.isArray(players) || !players.length) return '';
  const L = ROSTER_LABELS[lang] || ROSTER_LABELS.en;
  const th = 'text-align:left;font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;color:#71717a;padding:6px 10px;border-bottom:2px solid #e4e4e7';
  const td = 'font-size:0.88rem;padding:7px 10px;border-bottom:1px solid #f0f0f2';
  const rows = players.map(p => `<tr>
    <td style="${td}">${p.number || ''}</td>
    <td style="${td}">${p.name || ''}</td>
    <td style="${td}">${p.size || ''}</td>
    <td style="${td}">${p.qty || 1}</td>
  </tr>`).join('');
  return `<table cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;margin:4px 0 20px">
  <tr><th style="${th}">${L.number}</th><th style="${th}">${L.name}</th><th style="${th}">${L.size}</th><th style="${th}">${L.qty}</th></tr>
  ${rows}
</table>`;
}

function designBlock(design, lang) {
  const img = src => src ? `<img src="${src}" alt="" width="250" style="width:48%;max-width:250px;border:1px solid #e4e4e7;background:#fafafa;vertical-align:top">` : '';
  const imgs = (design.front || design.back)
    ? `<div style="margin:0 0 12px">${img(design.front)} ${img(design.back)}</div>` : '';
  return `${imgs}${rosterTable(design.players, lang)}`;
}

const CONFIRMED_3D = {
  en: (o, range) => ({
    subject: `Your MOMUTO kits are confirmed 🔥`,
    body: `<p style="font-size:0.95rem;line-height:1.8;margin:0 0 8px">Hi ${o.name}! Payment received — this is the design we're producing, exactly as you created it. Expect your kits around <strong>${range}</strong>.</p>`,
  }),
  es: (o, range) => ({
    subject: `Vuestras equipaciones MOMUTO están confirmadas 🔥`,
    body: `<p style="font-size:0.95rem;line-height:1.8;margin:0 0 8px">¡Hola ${o.name}! Pago recibido — este es el diseño que vamos a producir, exactamente como lo creaste. Os llegará en torno al <strong>${range}</strong>.</p>`,
  }),
  fr: (o, range) => ({
    subject: `Vos maillots MOMUTO sont confirmés 🔥`,
    body: `<p style="font-size:0.95rem;line-height:1.8;margin:0 0 8px">Salut ${o.name} ! Paiement reçu — voici le design que nous produisons, exactement comme vous l'avez créé. Comptez sur une livraison aux alentours du <strong>${range}</strong>.</p>`,
  }),
  it: (o, range) => ({
    subject: `Le vostre maglie MOMUTO sono confermate 🔥`,
    body: `<p style="font-size:0.95rem;line-height:1.8;margin:0 0 8px">Ciao ${o.name}! Pagamento ricevuto — questo è il design che produrremo, esattamente come l'hai creato. Contate su una consegna intorno al <strong>${range}</strong>.</p>`,
  }),
};

const TOTAL_LABELS = { en: 'Order total', es: 'Total del pedido', fr: 'Total de la commande', it: 'Totale ordine' };

exports.emailConfirmation3D = function(order) {
  const lang  = order.lang || 'en';
  const range = deliveryWindow(order.paidAt, 20, 25, lang);
  const { subject, body } = (CONFIRMED_3D[lang] || CONFIRMED_3D.en)(order, range);
  const designs = (order.designs || []).map(d => designBlock(d, lang)).join('');
  const total = order.total
    ? `<p style="font-size:0.9rem;margin:0 0 6px"><strong>${TOTAL_LABELS[lang] || TOTAL_LABELS.en}: ${order.total} ${order.currency || 'EUR'}</strong></p>`
    : '';
  return { subject, html: wrap(`${body}<div style="margin:18px 0 0">${designs}</div>${total}${ref(order)}${sig()}`) };
};
