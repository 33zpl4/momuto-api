...
/**
 * Create the "Polo collar" surcharge product on each store (EN/ES/FR/IT/US).
 *
 *   node scripts/create-collar-products.js                 # dry run (default)
 *   node scripts/create-collar-products.js --live          # create for real
 *   node scripts/create-collar-products.js --live --lang fr
 *
 * One 3.00 product per store (EUR; $3.00 on us). It is the billing vehicle
 * for the polo-collar DESIGN option (owner ruling 15 Sep 2026): the collar is
 * a property of the design, not a per-player choice, so a collared pre-designed
 * product is flagged by a marker in its store title ("Polo Collar" /
 * "Cuello Polo" / "Col Polo" / "Colletto Polo") and the design-server checkout
 * bills qty = number of jerseys on that line (jersey-only players included).
 * Flat +3 at every quantity tier — customer surfaces show resulting unit
 * prices (21,90 → 24,90), never a "+3 €" fee line; this product is the only
 * place the bare 3.00 exists. Same shape as create-long-sleeves-products.js.
 *
 * Deliberately:
...
const IDS_FILE = path.join(ROOT, 'cms', 'polo-collar', 'ids.json');

const PRICE = '3.00';
const HANDLE = 'polo-collar';
...
const IMAGE_SRC = process.env.COLLAR_IMAGE_SRC
...
const STORES = {
  en: {
    ...
    title: 'Polo collar',
    subtitle: 'Polo-collar jersey option',
    mini_detail: 'Polo-collar option — one unit per jersey of a polo-collar design. Added automatically from your cart.',
  },
  es: {
    tokenEnv: 'OEMSAAS_TOKEN_ES',
    title: 'Cuello polo',
    subtitle: 'Opción de camiseta con cuello polo',
    mini_detail: 'Opción de cuello polo: una unidad por cada camiseta de un diseño con cuello polo. Se añade automáticamente desde el carrito.',
  },
  fr: {
    tokenEnv: 'OEMSAAS_TOKEN_FR',
    title: 'Col polo',
    subtitle: 'Option maillot col polo',
    mini_detail: 'Option col polo : une unité par maillot d\'un design à col polo. Ajoutée automatiquement depuis le panier.',
  },
  it: {
    tokenEnv: 'OEMSAAS_TOKEN_IT',
    title: 'Colletto polo',
    subtitle: 'Opzione maglia con colletto polo',
    mini_detail: 'Opzione colletto polo: una unità per ogni maglia di un design con colletto polo. Aggiunta automaticamente dal carrello.',
  },
  // us.momuto.com bills in USD; the surcharge is $3.00 (same figure as long sleeves there).
  us: {
...
async function run() {
  ...
  for (const lang of langs) {
    ...
    console.log(`\n=== ${lang.toUpperCase()} — "${store.title}" @ ${lang === 'us' ? '$' : '€'}${PRICE} ===`);
    ...
    try {
      ...
      const ok = live && live.title === store.title && Number(price).toFixed(2) === PRICE && live.status === 1;
