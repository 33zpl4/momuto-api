 ...
 * The homepage band.
 *
 * This REPLACES pages/homepage/iconic-series.<lang>.html rather than adding a
 * second block — two Iconic bands on one page would compete for the same click.
 * Structure and CSS are reproduced from that file unchanged, so the section
 * still matches its siblings; only the cards and copy move to drop 02.
 *
 * Generated rather than hand-written for one reason: the image URLs. A mockup
 * re-render changes them, and four hand-maintained copies is exactly how a
 * stale URL survives unnoticed. Rebuild and re-paste instead.
 */
function renderHomepage(drop, all, lang) {
  const cfg = config.drops[drop]?.homepage;
  if (!cfg) throw new Error(`no homepage config for ${drop}`);
  const copy = cfg.copy?.[lang];
  const slugs = cfg.cards?.[lang];
  if (!copy || !slugs) throw new Error(`no ${lang} homepage copy`);

  const bySlug = new Map(all.filter(p => p.drop === drop).map(p => [p.slug, p]));
  const picks = slugs.map(s => {
    const p = bySlug.get(s);
    if (!p) throw new Error(`homepage card "${s}" is not a ${drop} product`);
    if (!p.image || !p.image_front) throw new Error(`${s} has no image/image_front — the hover swap needs both`);
    return p;
  });

  const handle = config.drops[drop].collection.handle;
  const cards = picks.map((p, i) => {
    const name = escapeHtml(p.display_title.toUpperCase());
    const alt = escapeHtml(p.display_title);
    // Card 3 is hidden on mobile by the existing CSS — same rule as drop 01.
    const cls = i === 2 ? 'iconic-card iconic-card--hide-mobile' : 'iconic-card';
    return [
      `      <!-- ${p.number} ${p.display_title} -->`,
      `      <a href="/products/${p.handle}" class="${cls}">`,
      '        <div class="iconic-card-image">',
      '          <div class="iconic-swap">',
      `            <img class="back" src="${p.image}" alt="${alt} – back">`,
      `            <img class="front" src="${p.image_front}" alt="${alt} – front">`,
      '          </div>',
      '        </div>',
      '        <div class="iconic-card-meta">',
      '          <div>',
  ...
  const css = fs.readFileSync(path.join(ROOT, 'pages', 'homepage', `iconic-series.${lang}.html`), 'utf8')
    .match(/<link rel="preconnect"[\s\S]*?<\/style>/);
  if (!css) throw new Error(`could not lift the styles out of pages/homepage/iconic-series.${lang}.html`);

  return [
    '<!-- ============================================================================',
    `     MOMUTO homepage block — ${lang.toUpperCase()} — Iconic Series ${config.drops[drop].label}`,
    '',
    '     GENERATED by scripts/build-iconic-pages.js. Do not hand-edit: the card',
    '     order and copy live in iconic-series/config.json under',
    `     drops.${drop}.homepage, and the image URLs come from the product JSONs.`,
    '',
    `     REPLACES the drop 01 version of this block — paste over it, don't add`,
    '     below it. Two Iconic bands compete for the same click.',
    '     ============================================================================ -->',
    '',
    css[0],
    '',
    '<section class="iconic-section">',
    '  <div class="iconic-inner">',
    '',
    '    <div class="iconic-header">',
    '      <div class="iconic-header-left">',
    `        <p class="iconic-eyebrow">${escapeHtml(copy.eyebrow)}</p>`,
    `        <h2 class="iconic-heading">${escapeHtml(copy.heading)}</h2>`,
    '      </div>',
    `      <a href="/collections/${handle}" class="iconic-view-all">${escapeHtml(copy.view_all)}</a>`,
    '    </div>',
    '',
    ...
    `      <p class="iconic-footer-desc">${escapeHtml(copy.footer_desc)}</p>`,
    `      <a href="/collections/${handle}" class="iconic-footer-cta">${escapeHtml(copy.cta)}</a>`,
    '    </div>',
    '',
    '  </div>',
    '</section>',
...
// Homepage band: one per locale, for the drops that define one.
for (const drop of drops) {
  if (!config.drops[drop]?.homepage) continue;
  for (const lang of langs) {
    try {
      const out = path.join(BUILD, 'homepage', `${drop}.${lang}.html`);
      fs.mkdirSync(path.dirname(out), { recursive: true });
      fs.writeFileSync(out, renderHomepage(drop, all, lang));
      console.log(`✓ ${path.relative(ROOT, out)}`);
      written++;
    } catch (err) {
      if (/no \w+ homepage copy/.test(err.message)) { skipped++; }
      else { console.error(`✗ homepage ${drop}.${lang}: ${err.message}`); failed++; }
    }
  }
}
